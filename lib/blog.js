...
function toUrlPath(s) {
  ...
  return normalized.replace(/\\/g, '/'); // handle windows
}
...
function initializeBlog() {
  ...
  // this.options.feedUrl = this.options.baseUrl + toUrlPath(this.options.feedUrl);
};
...
function buildPostPages(jadeData, templateData) {
  var templatePath = this.options.postTemplate,
      templateContents = fs.readFileSync(templatePath, {encoding: 'utf-8'}),
