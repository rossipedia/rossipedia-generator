...
function Post(blog, filename) {
  ...
  meta.urlpath = meta.filename.replace(blog.options.srcPattern, blog.options.replaceStr);
	if (meta.urlpath.charAt(0) !== '/') {
		meta.urlpath = '/' + meta.urlpath;
	}

  meta.destFile = path.join(blog.options.destDir, meta.urlpath);

  if (path.basename(meta.urlpath) === 'index.html') {
    meta.urlpath = path.dirname(meta.urlpath) + '/';
  }

  var root = blog.options.baseUrl;
